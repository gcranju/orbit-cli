...
async function initializeProgram(programId, provider) {
    ...
    if (!idl) {
        idl = JSON.parse(fs.readFileSync('/home/orbit/orbit-cli/lib/abc.json'));
    }
...
async function getConnectionAccounts(nid, xcallManagerId, provider, connection){
    let centralizedContracts = await fetchCentralizedContracts(xcallManagerId, provider);    
    if (connection){
        centralizedContracts = [connection]
    }
...
// console.log(findPda(["withdraw"], new PublicKey("Fn7RLz6BbL2TPouMY6nK9SWnLXdTfkM7A1mgvCppgvf1")));

console.log( findPda(['token_creation',new PublicKey('EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v')], new PublicKey('4u979CPSHUeJQbCYUAvoki4CQHDiG1257vt2DaJULPV9')))