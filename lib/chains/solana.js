...
async function handleMockMethods(method, params, connection, senderKeypair, chainConfig) {
    const mockProgramId = new PublicKey("3zfDW3nqaydahN5x7V8dqkZ1RSXesKAkvnWxShnvwpUC");
    ...
    switch (method) {
        case 'initialize':
            ...
            const initializeIx = await program.methods
                .initialize(new PublicKey("BknkMw3sonNXVTW2mzuhKEJmmt4PSVei1HbQiDuWVny4"))
                .accountsStrict({
                    config: configPda,
        ...
        case 'send_message':
            ...
            const connectionAccounts = await getConnectionAccounts("0x2.icon", xcallManagerId, createProvider());

            console.log(programSendMessage)
            // Create instruction for sending message
            const sendMessageIx = await programSendMessage.methods
                .sendCallMessage("0x2.icon/hxea3635f7495653d8596a7f23a78514b6ad1470e8", messageData, rollbackData)
                .accounts({
                    authority: authorityPdaSend,
                    connectionAccount: connectionPdaSend,
                    sender: senderKeypair.publicKey,
                    systemProgram: SystemProgram.programId,
...
async function executeCall(params, connection, senderKeypair, chainConfig) {
    ...
    const toNid = data_parts[3].split('/')[0]
    const associatedTokenAcc = new PublicKey(data_parts[4].split('/')[1])
