    ...
    callContractMethod: async function (contractName, method, params, env, options) {
        ...
        switch (contractName) {
            ...
            case 'asset-manager':
                ...
                break;
            case 'rate-limit':
                await handleRateLimitMethods(method, params, connection, senderKeypair, chainConfig);
                break;
            default:
...
async function handleAssetManagerMethods(method, params, connection, senderKeypair, chainConfig) {
    ...
    let connectionId = new PublicKey(params.connection);
    let rate_limit_program = new PublicKey(params.rate_limit)
    let hub_chain_id = params.hub_chain_id
    const hex = params.hub_asset_manager.startsWith('0x') ? params.hub_asset_manager.slice(2) : params.hub_asset_manager;
    console.log(`hex ${hex}`)
    let hub_asset_manager = Buffer.from(hex, 'hex')
    let connectionConfig = await calculatePdaSync(['config'], connectionId);
    console.log(`connectionConfig ${connectionConfig}, assetManagerId ${assetManagerId}, connectionId ${connectionId}, rate_limit_program ${rate_limit_program}, hub_chain_id ${hub_chain_id}, hub_asset_manager ${hub_asset_manager}`)
    ...
    // Helper function to calculate PDAs
    const calculatePdAs = async (...seeds) => calculatePdaSync(seeds, assetManagerId);
    const rateLimitConfig = await calculatePdaSync(['rmconfig'], rate_limit_program);
    switch (method) {
        case 'initialize':
            // Calculate PDAs for initialization
            const [configPda, authorityPDA, vaultNativePda] = await Promise.all([
                calculatePdAs('config-seed'),
                calculatePdAs('dapp_authority'),
                calculatePdAs('vault_native')
            ]);
            // Initialize program
            const program = await initializeProgram(assetManagerId, createProvider());

            // Create instruction for initialization
            const initializeIx = await program.methods
                .initialize(new anchor.BN(146), hub_asset_manager, connectionId, rate_limit_program, rateLimitConfig)
                .accountsStrict({
                    owner: senderKeypair.publicKey,
                    systemProgram: SystemProgram.programId,
                    config: configPda,
                    authority: authorityPDA,
                    nativeVaultAccount: vaultNativePda
                })
                .signers([senderKeypair.payer])
                .instruction();
...
async function handleRateLimitMethods(method, params, connection, senderKeypair, chainConfig) {
    const rateLimitId = new PublicKey(params.rate_limit);
    ...
    // Helper function to calculate PDAs
    const calculatePdAs = async (...seeds) => calculatePdaSync(seeds, rateLimitId);
    const rateLimitConfig = await calculatePdaSync(['rmconfig'], rateLimitId);
    switch (method) {
        case 'initialize':
            // Calculate PDAs for initialization
            const [configPda, authorityPDA, vaultNativePda] = await Promise.all([
            ...
            // Initialize program
            const program = await initializeProgram(rateLimitId, createProvider());
